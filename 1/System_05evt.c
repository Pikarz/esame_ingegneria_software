...
void System_function_initSample(DATA *data, threadData_t *threadData)
{
  ...
  data->modelData->samplesInfo[i].interval = ((modelica_real)(data->simulationInfo->integerParameter[23]/* mc.p.T PARAM */) ) /* (max int for single time events) */;
  i++;
}

const char *System_zeroCrossingDescription(int i, int **out_EquationIndexes)
{
  static const char *res[] = {"time > 4000.0"};
  static const int occurEqs0[] = {1,141};
...
int System_function_ZeroCrossings(DATA *data, threadData_t *threadData, double *gout)
{
  ...
  tmp0 = GreaterZC(data->localData[0]->timeValue, 4000.0, data->simulationInfo->storedRelations[0]);
  gout[0] = (tmp0) ? 1 : -1;
...
int System_function_updateRelations(DATA *data, threadData_t *threadData, int evalforZeroCross)
{
  ...
  if(evalforZeroCross) {
    tmp1 = GreaterZC(data->localData[0]->timeValue, 4000.0, data->simulationInfo->storedRelations[0]);
    data->simulationInfo->relations[0] = tmp1;
  } else {
    data->simulationInfo->relations[0] = (data->localData[0]->timeValue > 4000.0);
  }
